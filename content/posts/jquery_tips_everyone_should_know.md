...
description: A collection of simple tips to help up your jQuery game.
tags:
  - jquery
  - javascript
  - tips
...
- [Contribution Guidelines](CONTRIBUTING.md.html)
...
You can also check if one particular image has loaded by replacing the `` tag with an ID or class.
...
But all of those `val()` calls are expensive and using `.val()` on `` elements will strip carriage return characters from the browser-reported value. A better way of collecting user inputs is using the `serialize()` function which collects them as a string:
