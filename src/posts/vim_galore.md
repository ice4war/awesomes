...
### [List of plugins](PLUGINS.md.html)
...
Unmap it again by using `:nunmap `.
...
normal mappings and `:nmap ` shows all normal mappings that start with
the mapleader.
...
This mapping is triggered by `\h`. If you want to use `h` instead:
...
Moreover, there is `` that is the local counterpart to ``
...
| Expression | `=` | user | [ ] | Evaluation of the VimL expression that was yanked. E.g. do this in insert mode: `=5+5` and "10" will be inserted in the buffer. |
...
see mappings like `:vnoremap foo :command`. Here `` is used to remove
...
| `` | keywords from `'dictionary'` option | `:h i^x^k` |
| `` | keywords from `'thesaurus'` option | `:h i^x^t` |
| `` | keywords from current and included files | `:h i^x^i` |
| `` | tags | `:h i^x^]` |
| `` | file names | `:h i^x^f` |
| `` | definitions or macros | `:h i^x^d` |
| `` | Vim commands | `:h i^x^v` |
| `` | user defined (as specified in `'completefunc'`) | `:h i^x^u` |
| `` | omni completion (as specified in `'omnifunc'`) | `:h i^x^o` |
| `s` | spelling suggestions | `:h i^Xs` |
...
(next) and `` (previous), which also happen to be the keys used for
...
`/` is a motion. They also take a count. `2?the` jumps to the second last
occurrence of "the".
...
Undo (`u`) and redo (``) work branch-wise. They go up and down the current
...
goes back to the state of node "bar" and another `` to the state of node
...
In the quickfix buffer `` opens the selected entry in the associated window
and `` in a new window.
...
(The same could be done without macros: `oabc10.`)
...
You can use `` (this is ctrl+d) to list all tags that
match the currently entered query. E.g. `:h tab` will get you a list of all
tags from `tab` over `'softtabstop'` to `setting-guitablabel`.

You want to list all VimL functions? Simple: `:h ()`. You want to list all
VimL functions that concern windows? `:h win*()`.
...
navigate to an entry and hit `` to jump to that match. See `:h quickfix` for
the whole truth.
...
Switch to visual block mode with ``. Afterwards go down for a few lines.
...
after the end of each line, do this: `3j$Atext`.
...
```vim
nnoremap  cs :cscope find s =expand('')
nnoremap  cg :cscope find g =expand('')
nnoremap  cc :cscope find c =expand('')
nnoremap  ct :cscope find t =expand('')
nnoremap  ce :cscope find e =expand('')
nnoremap  cf :cscope find f =expand('')
nnoremap  ci :cscope find i ^=expand('')$
nnoremap  cd :cscope find d =expand('')
```

So, when `:tag` (or ``) jumps to a definition from the tags file, `:cstag`
...
xnoremap  n 'Nn'[v:searchforward]
onoremap  n 'Nn'[v:searchforward]
...
`` and `` respectively. By default, this also works in the
command-line and recalls older or more recent command-lines from history.

So far, so good. But `` and `` are even smarter! They recall the
command-line whose beginning matches the current command-line. E.g. `:echo `
...
By default, `` clears and redraws the screen (like `:redraw!`). The
...
```vim
nnoremap l :nohlsearch:diffupdate:syntax sync fromstart
```
...
Now `5[` inserts 5 blank lines above the current line.

## Quickly edit your macros

This is a real gem! The mapping takes a register (or `*` by default) and opens
it in the cmdline-window. Hit `` when you're done editing for setting the
register.

I often use this to correct typos I did while recording a macro.

```vim
nnoremap m :='let @'. v:register .' = '. string(getreg(v:register))
```

Use it like this `m` or `"qm`.

Notice the use of `` to make sure that the `` is inserted
...
if empty($TMUX)
 ...
 let &t_SR = "\Ptmux;\\]50;CursorShape=2\x7\\\"
endif
```
...
If you select one or more lines, you can use `` for shifting them
...
The keyword completion (``/``) tries completing whatever is listed in
...
Useful commands that are good to know. Use `:h :` to learn more
...
As you can see, using `` will repeat the previous debugger command, `s` in
this case.
...
```vim
:syntime on
" hit  a few times to redraw the window which causes the syntax rules to get applied again
:syntime off
:syntime report
```
...
- Vim expects Alt (also called _Meta key_) to send a proper 8-bit
...
You can test the above like this: `vim -u NONE -N` and type `i` and
