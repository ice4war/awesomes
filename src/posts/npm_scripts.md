---
title: "Npm Scripts"
description: "title: Awesome npm Scripts"
tags: ['javascript', 'js', 'npm']
---


---
title: Awesome npm Scripts
description: Everything awesome for using npm as a build tool.
tags:
  - npm
  - scripts
  - build-tool
...
- [watch](https://github.com/mikeal/watch) - `watch  `.
...
- Use `cd ` to change the current working directory to ``. Note that `cd` alone prints the current working directory on windows, but changes the working directory to `~` on \*nix.
...
- You can run `npm run  -s` to silence the default npm output (useful for calling a script within another script).
