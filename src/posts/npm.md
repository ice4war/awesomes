---
title: Awesome npm
description: Awesome npm resources and tips. npm is a package manager for the JavaScript programming language.
tags:
  - npm
  - nodejs
  - package-manager
...
# Awesome npm [![Awesome](https://awesome.re/badge.svg)](https://awesome.re) [](https://www.npmjs.com)
...
You can easily [run scripts](https://docs.npmjs.com/cli/run-script) using npm by adding them to the `"scripts"` field in package.json and run them with `npm run `. Run `npm run` to see available scripts. Binaries of locally install packages are made available in the [PATH](https://en.wikipedia.org/wiki/PATH_(variable)), so you can run them by name.
